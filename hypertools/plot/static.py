...
def static_plot(x, *args, **kwargs):
    ...
    ##SUB FUNCTIONS##
    def dispatch(data):
        if data[0].ndim == 1 or data[0].shape[-1] == 1:
            return plot1d(data)
        elif data[0].shape[-1] == 2:
            return plot2d(data)
        elif data[0].shape[-1] == 3:
            return plot3d(data)
    ...
    def annotate_plot(data, labels):
        ...
        Args:
            data (np.array) - array of points, of shape (numPoints, 3)
        ...
        if kwargs['explore']:
            ...
            if labels:
                ...
                fig.canvas.mpl_connect('motion_notify_event',
                                       lambda event: onMouseMotion(event, X, labels))  # on mouse motion
    ...
    def annotate_plot_explore(X, index, labels=False):
        ...
        if type(labels) is list:
            ...
            label = "Index " + str(index) + ": (" + "{0:.2f}, ".format(X[index, 0]) + "{0:.2f}, ".format(
                X[index, 1]) + "{0:.2f}".format(X[index, 2]) + ")"
