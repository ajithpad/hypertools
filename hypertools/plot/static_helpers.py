...
def plot_static(x, args_list, kwargs_list):
	if x[0].ndim==1 or x[0].shape[-1]==1:
		return plot1D(x, args_list, kwargs_list)
	elif x[0].shape[-1]==2:
		return plot2D(x, args_list, kwargs_list)
	elif x[0].shape[-1]==3:
		return plot3D(x, args_list, kwargs_list)

# plot data in 1D
def plot1D(data, args_list, kwargs_list):
	n=len(data)
	fig, ax = plt.subplots()
...
# plot data in 2D
def plot2D(data, args_list, kwargs_list):
	n=len(data)
	fig, ax = plt.subplots()
	for i in range(len(data)):
		iargs = args_list[i]
...
# plot data in 3D
def plot3D(data, args_list, kwargs_list):
	fig = plt.figure()
	ax = p3.Axes3D(fig)
	for i in range(len(data)):
		iargs = args_list[i]
